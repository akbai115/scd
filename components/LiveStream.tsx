...
export const LiveStream: React.FC = () => {
    ...
    return (
        <div className="absolute inset-0 flex flex-col items-center overflow-hidden font-sans text-black animate-in fade-in duration-700 pointer-events-auto">

            <div className="flex w-full h-full pt-56 px-6 md:px-12 gap-8">
                {/* SIDEBAR */}
                <div className="hidden md:flex flex-col w-64 h-full border-r border-black/10 pr-8 gap-8 opacity-60 hover:opacity-100 transition-opacity duration-500">
                    <h1 className="times-bold text-2xl tracking-tighter uppercase mb-4">YEEZYSTREAM</h1>

                    <nav className="flex flex-col gap-6">
                        <button
                            onClick={() => setActiveTab('HOME')}
                            className={`flex items-center gap-4 text-[10px] font-bold tracking-[0.2em] uppercase transition-all ${activeTab === 'HOME' ? 'opacity-100 decoration-black underline underline-offset-4' : 'opacity-40 hover:opacity-100'}`}
                        >
                            HOME
                        </button>
                    </nav>
                ...
                <div className="flex-1 h-full overflow-y-auto pb-32 no-scrollbar">
                    ...
                    {/* FEATURED VIDEO */}
                    <div className="w-full aspect-video bg-black/5 border border-black/10 relative mb-12 group cursor-pointer overflow-hidden backdrop-blur-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {VIDEO_FEED.map((video) => (
                            <div key={video.id} className="flex flex-col gap-3 group cursor-not-allowed opacity-50">
                                <div className="w-full aspect-video bg-black/5 border border-black/5 relative overflow-hidden backdrop-blur-[2px] grayscale">
                                    <div className="absolute inset-0 flex items-center justify-center z-10">
                                        <div className="bg-black/80 text-white text-[8px] font-bold px-2 py-1 tracking-widest uppercase">LOCKED</div>
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <div className="w-6 h-6 rounded-full border border-black/10 flex-shrink-0 grayscale" />
