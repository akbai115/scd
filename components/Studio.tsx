    ...
    return (
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-auto transition-all duration-1000 animate-in fade-in zoom-in-95 px-16">
            <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-4 gap-12 lg:gap-20 items-center">
                ...
                <div className="hidden lg:flex flex-col gap-10 border border-white/5 bg-white/[0.01] p-10 text-white">
                    <header className="flex flex-col gap-2">
                        <span className="text-[10px] font-bold tracking-[1em] opacity-40 uppercase">SAMPLING_RAW</span>
                        <div className="w-full h-[1px] bg-white/10" />
                    </header>
                    <div className="relative h-72 border border-white/20 flex items-center justify-center overflow-hidden bg-white/[0.03]">
                        <div className="flex items-end gap-[1px] h-full w-full p-2">
                            {[...Array(24)].map((_, i) => (
                                <div
                                    key={i}
                                    className="flex-1 bg-white/60 transition-all duration-75"
                ...
                <div className="lg:col-span-2 flex gap-8 md:gap-12 items-end justify-center">
                    {Object.entries(stems).map(([key, active]) => (
                        <div
                            key={key}
                            className="flex flex-col items-center group cursor-crosshair flex-1 max-w-[200px] text-white"
                            onClick={() => { onToggleStem(key as any); onAction(); }}
                        >
                            <div className={`w-full aspect-[1/3.5] border border-white/20 relative transition-all duration-1000 shadow-2xl flex flex-col justify-between p-8
                ${active ? 'bg-white/[0.1] -translate-y-12 shadow-[40px_40px_100px_rgba(255,255,255,0.15)]' : 'bg-transparent opacity-15 translate-y-6 scale-95'}
                `}>
                                <div className="w-full h-[2px] bg-white/20 mb-12" />
                                ...
                                <div className="flex flex-col gap-4 [writing-mode:vertical-lr] rotate-180 items-center justify-center flex-1">
                                    ...
                                    <div className={`w-[2.5px] h-48 transition-all duration-1000 ${active ? 'bg-white/60 h-64' : 'bg-white/5'}`} />
                                </div>

                                <div className="w-full flex justify-center mt-12">
                                    <div className={`w-14 h-14 border-2 border-white/20 flex items-center justify-center transition-all duration-1000
                    ${active ? 'bg-white rotate-45 scale-110' : 'bg-transparent'}
                    `}>
                                        {active && <div className="w-4 h-4 bg-black/40 rounded-full blur-[2px] animate-pulse" />}
                                    </div>
                                </div>
                            </div>
                ...
                <div className="hidden lg:flex flex-col gap-12 border-l border-white/10 pl-16 text-white">
                    <header className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-4 mt-8">
                        <button
                            onClick={() => { onAction(); }}
                            className="w-full py-10 border-2 border-white flex items-center justify-center group relative overflow-hidden transition-all hover:bg-white hover:text-black text-white active:scale-95"
                        >
            ...
            <div className="mt-48 text-center max-w-4xl opacity-15 text-white">
                <div className="w-48 h-[1px] bg-white/20 mx-auto mb-16" />
